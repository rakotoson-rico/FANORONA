...
def dessin_plateau(event=None):
    ...
    if tour == "noir":
        ...
        couleur_tour = "black"  # nampiana : rehefa an'ny mainty ny tour, mainty ny soratra
    else:
        texte_tour = "An'ireo fotsy izao ! "
        couleur_tour = "white"  # nampiana : rehefa an'ny fotsy ny tour, fotsy ny soratra
    ...
    if pion_selectionne is not None:
        for colonne, ligne in lalana_fidina():
            ...
            canvas.create_oval(x - 7, y - 7, x + 7, y + 7, fill="#ffd400", outline="#ffffff", width=1)
    ...
    if btn_miverina is not None:
        canvas.create_window(
            ...
            hauteur - 30,          # 30 pixel eo ambony ny bord ambany
            window=btn_miverina,
            anchor="center"
        )
    ...
    if btn_apody is not None:
        canvas.create_window(
            largeur / 2 + 130,     # eo ankavanana indrindra hoan'ny APODY
            hauteur - 30,          # 30 pixel eo ambony ny bord ambany
            window=btn_apody,
            anchor="center"
        )
...
def lancer():
    ...
    btn_apody = ttk.Button(canvas, text="APODY", style="Apody.TButton", command=annuler_dernier)
    btn_miverina = ttk.Button(canvas, text="HIVOKA", style="Apody.TButton", command=retour_menu)

    btn_vaovao = ttk.Button(canvas, text="VAOVAO", style="Apody.TButton", command=lalao_vaovao)
    style = ttk.Style()
...
def menu():
    ...
    for tag, texte, position_y in boutons:
        ...
        canvas.create_text(0, 0, text=texte, font=("Arial", 20, "bold"), fill=couleur_bouton, tags=tag)
    canvas.create_text(0, 0, text="◆", font=("Arial", 12), fill="#9b5d3d", tags="separateur1")
    canvas.create_text(0, 0, text="◆", font=("Arial", 12), fill="#9b5d3d", tags="separateur2")
    canvas.create_text(0, 0, text="◆", font=("Arial", 12), fill="#9b5d3d", tags="separateur2")
    canvas.create_text(0, 0, text="◆", font=("Arial", 12), fill="#9b5d3d", tags="separateur3")

    def redimensionner_menu(event=None):
        largeur = max(canvas.winfo_width(), 1)
        hauteur = max(canvas.winfo_height(), 1)
        ratio = max(largeur / image_fond_originale.width, hauteur / image_fond_originale.height)
        image = image_fond_originale.resize(
            (int(image_fond_originale.width * ratio), int(image_fond_originale.height * ratio)),
...
def comment_jouer():
    ...
    canvas.create_text(0, 0, text=regle_jeu, font=("Georgia", 14), fill=couleur_texte,
                        width=650, justify="left", tags="regle_jeu")

    # boutton retour
    canvas.create_rectangle(0, 0, 0, 0, fill=couleur_texte, outline="", tags="fond_retour")
    canvas.create_text(0, 0, text=" MIVERINA ", font=("Georgia", 14, "bold"), fill=couleur_accent,
                        anchor="w", tags="bouton_retour")
...
def sauvegarder_etat(): 
    global historique, plateau, tour, pion_selectionne, positions_visitees, derniere_direction, choix_capture
...
def pions_selectionnables():
    ...
    for ligne in range(lignes_plateau):
        for colonne in range(colonnes_plateu):
            ...
            if not coups:
                continue
            if not doit_capturer:
                selectionnables.add((colonne, ligne))
            elif any(coups.values()):
                selectionnables.add((colonne, ligne))
